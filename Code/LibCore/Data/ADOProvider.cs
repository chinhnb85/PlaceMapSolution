...
namespace LibCore.Data
{
    ...
    public class ADOProvider : IDisposable
    {
        ...
        /// <returns></returns>
        public bool ProcedureExecute(string procedure, object oParams = null,IDbTransaction transaction = null)
        {
            try
            {
                using (MiniProfiler.Current.Step(procedure))
                {
                    using (Connection)
                    {
                        Connection.Execute(procedure, oParams, transaction: transaction, commandType: CommandType.StoredProcedure);
                    }
                }
                return true;
            }
